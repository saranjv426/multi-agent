...
async def startup_event():
    ...
    try:
        ...
        logger.info(
            f"✅ Optimized validators initialized successfully with models: {', '.join(sorted(optimized_validators.keys()))}"
        )
...
async def get_system_status(user: Dict[str, Any] = Depends(require_auth)):
    ...
    status["available_models"] = sorted(optimized_validators.keys())
...
async def validate_roof_design_optimized(
    ...
    try:
        ...
        logger.info(f"Starting optimized validation for: {file.filename} with model: {normalized_model}")
